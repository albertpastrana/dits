...
impl Ticket {
    ...
    /// It can also return any error returned by `Ticket::from_path`
    pub fn find_ticket(workdir: &Path, tickets_dir: &Path, name: &String) -> Result<Vec<Ticket>, CliError> {
      let res = try!(run_cmd("find", &[&tickets_dir.to_string_lossy(), "-type", "f", "-iname", name]));
      let vec: Result<Vec<_>, _> = res.split('\n')
                   .filter(|s| !s.is_empty())
                   .map(|s| Ticket::from_path(workdir, tickets_dir, s))
                   .collect();
      vec
    }
    ...
    pub fn from_path(workdir: &Path, tickets_dir: &Path, ticket_path: &str) -> Result<Ticket, CliError> {
        ...
        let fname: Vec<&str> = try!(rtp.file_stem()
                                       .and_then(|osstr| osstr.to_str())
                                       .map(|s| s.rsplitn(2, '.').collect())
                                       .ok_or(CliError::PathError("Can't get file stem.")));
        if fname.len() != 2 {
        ...
        let parent = try!(location.parent()
                                  .and_then(|p| p.to_str())
                                  .ok_or(CliError::PathError("Can't get parent filename.")));
        Ok(Ticket {
